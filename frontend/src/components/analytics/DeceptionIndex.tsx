...
import type { DeceptionIndexEntry } from "@/types/analytics";
...
interface DeceptionIndexProps {
  data: DeceptionIndexEntry[];
}

export default function DeceptionIndex({ data }: DeceptionIndexProps) {
  const baselineEntry = data.find((d) => d.profile === "baseline");
  const baselineValue = baselineEntry?.deception_index ?? 0;
    ...
    <ResponsiveContainer width="100%" height={360}>
      ...
      >
        ...
        <Tooltip
          ...
          formatter={(value) => [Number(value).toFixed(3), "Deception Index"]}
        />
        {baselineValue > 0 && (
          <BaselineOverlay value={baselineValue} label="Baseline" />
        )}
        <Bar
          dataKey="deception_index"
          radius={[4, 4, 0, 0]}
          maxBarSize={48}
        >
